...
*/
static int
SDLTest_RunTest(SDLTest_TestSuiteReference *testSuite,const SDLTest_TestCaseReference *testCase,Uint64 execKey,
...
/* Test did not return a TEST_COMPLETED value; assume it failed */
SDLTest_LogError(SDLTEST_FINAL_RESULT_FORMAT,"Test",testCase->name,
                 "Failed (test started, but did not return TEST_COMPLETED)");
} else if(testCaseResult == TEST_ABORTED){
/* Test was aborted early; assume it failed */
...
runResult=1;
SDLTest_LogError(SDLTEST_LOG_SUMMARY_FORMAT,"Run",countSum,totalTestPassedCount,totalTestFailedCount,
                 totalTestSkippedCount);
