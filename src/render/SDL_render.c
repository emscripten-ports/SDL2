...
aligner=(alignment && ((renderer->vertex_data_used % alignment) != 0)) ? (alignment-
                                                                          (renderer->vertex_data_used % alignment)) : 0;
...
if(!renderer->viewport_queued ||
   (SDL_memcmp(&renderer->viewport,&renderer->last_queued_viewport,sizeof(SDL_Rect)) != 0)){
...
static void
GetWindowViewportValues(SDL_Renderer *renderer,int *logical_w,int *logical_h,SDL_Rect *viewport,SDL_FPoint *scale){
...
SDL_SetError("Texture dimensions are limited to %dx%d",renderer->info.max_texture_width,
